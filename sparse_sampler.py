...
def outter_product_indexing(base_vectors): # one vector per point and per dimension
    ...
    # https://stackoverflow.com/questions/40601372/how-to-efficiently-compute-the-column-wise-outer-vector-in-numpy
...
class bs_SparseSampler:
    ...
    def __init__(self, coeffs_shape, sample_coords, measuring_grid, basis, support, bcs, sparse_library=False, type = np.float64):
        ...
        # degree = 3; support = degree+1
        # sample_coords = np.array([[1.3,2.4,1.2,3.2],[3.7,4.1,4.1,6.3], [5.2,6.3,8.3,7.2]])[:,0:4]
        # bc_behavior= np.array([10,100,10,100])
        # measuring_grid=None
        # import scipy.signal as sg; basis = lambda args: sg.bspline(args, 3)
        # assuming samples as [[x,y],..., [x,y]] and assuming uniform grid for the basis
        self.sparse_library = sparse_library
        if sparse_library:
            import sparse as sp
        else:
            from logsplines import scipy_lib; sp = scipy_lib.sparse
        self.sp = sp
        self.bcs = bcs
...
if __name__ == '__main__':
    ...
    #period = float(domain_offset[0]+domain_length[0])
    #bcs_behaviour = bu.bcs_to_value(bcs, periods=period, domain_shape=coeffs_shape)
    ...
    # v=np.random.rand(*coeffs_shape)
    # SS = sparse_constructor((ii, *jjkk), Sijk, (n_samples,)+coeffs_shape)
    # SS_adj = SS.transpose()
    # pp = sp.tensordot(SS, v, axes=4)
    # rr = sp.tensordot(SS_adj, pp, axes=1).T

    # jj = np.ravel_multi_index(jjkk, coeffs_shape)
    # import scipy.sparse as scps
    # SY = scps.coo_matrix((Sijk, (ii, jj)), shape=(n_samples,)+ (np.prod(coeffs_shape),))
    # SY_adj = SY.transpose()
    # SY*v.flatten()-pp

    # np.max((SY_adj*pp).reshape(coeffs_shape)-rr)
    # np.max(rr)
